...
(start_time, end_time, forecasts, metrics, variables, grids,
 ...
 recompute, backend, remote_name, remote, remote_config) = parse_args(agg_only=True)
...
def run_grouped(combo):
    """Run grouped metrics."""
    print(combo)
    metric, variable, grid, region, lead, time_grouping = combo

    try:
        grouped_metric(start_time, end_time, variable, lead, "era5", "ghcn", metric,
