...
@cacheable(data_type='array',
           ...
           chunking={"lat": 300, "lon": 400, "forecast_date": 300, 'lead': 1, 'quantiles': 1},
           auto_rechunk=False)
def salient_blend_bilinear(start_time, end_time, variable, timescale="sub-seasonal", grid="global0_25"):  # noqa: ARG001
    """Processed Salient forecast files."""
    ds = salient_blend_raw(variable, timescale=timescale)
    ds = ds.dropna('forecast_date', how='all')

    # Regrid the data
    ds = regrid(ds, grid, base='base180', method='linear')
    return ds
...
@cacheable(data_type='array',
           ...
           cache=False,
           cache_args=['variable', 'lead', 'prob_type', 'grid', 'mask', 'region'])
def salient_linear(start_time, end_time, variable, lead, prob_type='deterministic',
                   grid='global0_25', mask='lsm', region='africa'):
    """Standard format forecast data for Salient."""
    lead_params = {
        "week1": ("sub-seasonal", 1),
    ...
    ds = salient_blend_bilinear(start_time, end_time, variable, timescale=timescale, grid=grid)
    ds = ds.sel(lead=lead_id)
    if prob_type == 'deterministic':
        # Get the median forecast
        ds = ds.sel(quantiles=0.5)
