"""Functions to fetch and process data from the S2S IRI model datasets."""
import sys
import pandas as pd
...
@cacheable(data_type='array',
           cache_args=['time', 'model', 'variable', 'forecast_type', 'run_type', 'grid'],
           chunking={'lat': 121, 'lon': 240, 'lead_time': 46, 'model_run': 1,
                     'start_date': 969, 'model_issuance_date': 1})
def single_iri_model(time, model, variable, forecast_type,
                     run_type="average", grid="global1_5",
                     verbose=True):
    """Fetches forecast data from the IRI <model> dataset.

    Args:
        time (str): The date to fetch data for (by day).
        model (str): The model to fetch data from. One of:
            ecmwf, ncep, jma, ukmo, cma, ncep, kma
    ...
    leads_id = "LA" if variable == "tmp2m" else "L1"

    if model == "ecmwf":
        model_name_on_server = "ECMF/.CY49"
        # Custom lead naming for ECMWF
        leads_id = "LA" if variable == "tmp2m" else "L"
    elif model == "ncep":
        model_name_on_server = "NCEP"
        leads_id = "LB" if variable == "tmp2m" else "L1"
        if forecast_type == "reforecast":
            raise NotImplementedError("NCEP reforecasts are not yet configured.")
    elif model == "jma":
        model_name_on_server = "JMA"
        if forecast_type == "reforecast":
            raise NotImplementedError("JMA reforecasts are not yet configured.")
    elif model == "ukmo":
        model_name_on_server = "UKMO"
        # Custom configs for UKMO
        if variable == "tmp2m":
            weather_variable_name_on_server = "1p5m_above_ground/.2t"
        if forecast_type == "reforecast":
            raise NotImplementedError("UKMO reforecasts are not yet configured.")
    elif model == "cma":
        model_name_on_server = "CMA/.BCC-CPS-S2Sv2"
        if forecast_type == "reforecast":
            raise NotImplementedError("CMA reforecasts are not yet configured.")
    elif model == "kma":
        model_name_on_server = "KMA/.r2p5"
        if forecast_type == "reforecast":
            raise NotImplementedError("KMA reforecasts are not yet configured.")
    else:
    ...
    if variable == "tmp2m":
        ...
        differences_url = f"{leads_id}+differences/"
    ...
    URL = (
        f"https://iridl.ldeo.columbia.edu/SOURCES/.ECMWF/.S2S/.{model_name_on_server}/"
    ...
    if r.status_code == 200 and r.headers["Content-Type"] == "application/x-netcdf":
        ...
        print(f"Data for {day} {month} {year} is not available for model {model}.\n")
        return None
    else:
        raise ValueError(f"Failed to download data for {day} {month} {year} for model {model}.")
